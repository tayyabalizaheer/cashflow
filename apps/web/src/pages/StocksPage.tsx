...
export function StocksPage() {
  ...
  const stockPath = useMemo(() => {
    ...
    if (appliedStockNames.length > 0) params.set("stockNames", appliedStockNames.join(","));
    if (appliedAverageByStock) params.set("average", "true");
    return `/stocks?${params.toString()}`;
  }, [appliedAverageByStock, appliedFromDate, appliedStockNames, appliedToDate, page, pageSize, search]);

  const query = useQuery({
    queryKey: ["stocks", stockPath],
    queryFn: () =>
      api<{ data: Stock[]; meta: { page: number; pageSize: number; total: number; average?: boolean; latestValidityDate?: string | null } }>(
        stockPath,
        { onlineOnly: true }
      )
  });
  const optionsQuery = useQuery({
    ...
    queryFn: () => api<{ data: StockOption[] }>("/stocks/options", { onlineOnly: true })
  });
  ...
  const filteredStockOptions = useMemo(() => {
    ...
    return stockOptions.filter((option) =>
      [option.fundName, option.category].some((value) => value?.toLowerCase().includes(needle))
    );
  ...
  function toggleStockName(fundName: string) {
    setDraftStockNames((current) =>
      current.includes(fundName) ? current.filter((item) => item !== fundName) : [...current, fundName]
    );
  }
    ...
    <section className="page stocks-page">
      <header className="page-header records-header">
        ...
        <div className="stock-header-actions">
          ...
          <button className="icon-button" type="button" title="Filter stocks" onClick={openFilters}>
      ...
      {showFilters ? (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Stock filters">
          <section className="modal-panel stock-filter-modal">
            <div className="modal-header">
              ...
              <button className="icon-button" type="button" title="Close" onClick={() => setShowFilters(false)}>
                <X size={18} />
              </button>
            </div>
            <div className="stock-filter-content">
              ...
              <label className="stock-average-toggle">
                <input
                  ...
                  onChange={(event) => setDraftAverageByStock(event.target.checked)}
                />
                <span>
                  <strong>Average by stock</strong>
                  <small>Group matching records and average price and return values.</small>
                </span>
              </label>
                ...
                {draftStockNames.length > 0 ? (
                  <div className="stock-chip-list" aria-label="Selected stocks">
                    {draftStockNames.map((fundName) => (
                      <button className="stock-chip" type="button" key={fundName} onClick={() => toggleStockName(fundName)}>
                        <span>{fundName}</span>
                        <X size={14} />
                      </button>
                    ))}
                ...
                <div className="stock-select-dropdown">
                  ...
                  >
                    <span>{draftStockNames.length > 0 ? `${draftStockNames.length} stock(s) selected` : "Choose stocks"}</span>
                    <ChevronDown size={17} />
                  </button>
                  {stockDropdownOpen ? (
                    <div className="stock-select-panel">
                      <label className="stock-select-search">
                        ...
                        <input
                          ...
                          onChange={(event) => setStockOptionSearch(event.target.value)}
                        />
                      </label>
                      <div className="stock-option-grid">
                        {filteredStockOptions.map((option) => (
                          <label className="stock-option" key={option.fundName}>
                            <input
                              type="checkbox"
                              checked={draftStockNames.includes(option.fundName)}
                        ...
                        {filteredStockOptions.length === 0 ? <div className="empty-state">No stocks match your search.</div> : null}
                      </div>
                    </div>
                  ) : null}
                </div>
            ...
            <div className="stock-filter-footer">
              <button className="secondary-button" type="button" onClick={clearDraftFilters}>
                Clear
              </button>
      ...
      {query.error ? <div className="form-error">Could not load stocks.</div> : null}
      {query.isLoading ? <div className="empty-state">Loading stocks...</div> : null}
      ...
      {stocks.length > 0 ? (
        <>
                  ...
                  >
                    <td>
                      ...
                      <span>
                        ...
                        {stock.isAverage ? ` | ${stock.recordCount ?? 0} records averaged` : ""}
                      </span>
                    </td>
          ...
          <div className="stock-card-list">
            {stocks.map((stock) => (
              <button className="stock-card" type="button" key={stock.id} onClick={() => setSelectedStock(stock)}>
          ...
          <div className="pagination-bar">
            <button className="secondary-button compact" type="button" disabled={page <= 1} onClick={() => setPage((value) => value - 1)}>
              Previous
            </button>
            <span>
      ...
      {selectedStock ? (
        <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="Stock details">
          <section className="modal-panel stock-detail-modal">
            <div className="modal-header">
              ...
              <button className="icon-button" type="button" title="Close" onClick={() => setSelectedStock(null)}>
                <X size={18} />
              </button>
            </div>
